...
The main point of the result bound is that the CAD algorithm is doubly experiential on a number of input polynomials and number of variables. The exponential behavior, As was proved in \citetitle{DAVENPORT198829}, is not restricted to the algorithm itself but to the problem it solves. 
...
When we defined the notation of the sub-CAD, then we can introduce a rule, how we choose regions into our sub-CAD. In \citetitle{Wilson2014} they introduced notation of variety sub-CAD $\ell$-sub-CAD as follow:
...
The better computing time in practice resides in the theoretical upper bound for computing only the full-dimensional CAD, which is significantly lower as the original Collins upper bound. As is shown in  \citetitle{10.1093/comjnl/36.5.432}, the upper bound for full-dimensional CAD is:
...
We tested the overall functionality of our implementation, by comparing the results with already computed  decomposition described in \citetitle{Arnon:1984:CAD:2054.2069} 
